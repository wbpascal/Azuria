...
namespace Azuria.Test
{
	...
	public class ConferenceTest
	{
	    ...
	    public async Task LeaderTest()
	    {
            ...
            var lLeader = await this._conference.Leader.GetObject(User.System);
        ...
        public async Task ParticipantsTest()
        {
            ...
            var lParticipants = await this._conference.Participants.GetObject(new User[0]);
        ...
        public async Task TitleTest()
        {
            ...
            var lTitle = await this._conference.Title.GetObject(lRandomHexString);
        ...
        public async Task FavouriteTest()
        {
            ...
            var lIsFavouriteResult = await this._conference.IsFavourite.GetObject();
            Assert.IsTrue(lIsFavouriteResult.Success);
            var lSetFavouriteResult = await this._conference.IsFavourite.SetObject(!lIsFavouriteResult.Result);
        ...
        public async Task BlockTest()
        {
            ...
            var lIsBlockedResult = await this._conference.IsBlocked.GetObject();
            Assert.IsTrue(lIsBlockedResult.Success);
            var lSetBlockedResult = await this._conference.IsBlocked.SetObject(!lIsBlockedResult.Result);
	    ...
	    public async Task MessagesTest()
	    {
            ...
            this._conference.NeuePmRaised += async (sender, messages, nachrichten) =>
            {
                ...
                if (nachrichten)
                {
                    var lSendMessageResult = await this._conference.SendMessage(lHexString);
